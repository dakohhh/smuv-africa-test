...
export class TokenService {
  async generateAuthToken(user: Pick<IUser, "_id" | "role">) {
  ...
  async generateVerificationAndResetToken({ userId, token_type }: { userId: string; token_type: VERIFICATION_TOKEN_TYPE }) {
  ...
  // to generate otp
  async generateOtpVerificationToken({ userId, token_type }: { userId: string; token_type: VERIFICATION_TOKEN_TYPE }) {
    // Find the previous token if its exists and delete it
    await VerificationToken.findOneAndDelete({ user: userId, type: token_type });
  ...
  async verifyEmailToken({ userId, _token, token_type }: { userId: string; _token?: string; token_type: VERIFICATION_TOKEN_TYPE }): Promise<boolean> {
  ...
  async verifyOtpToken(): Promise<void> {}
}
