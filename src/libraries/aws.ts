...
import { S3Client, PutObjectCommand, GetObjectCommand, DeleteObjectCommand, CompleteMultipartUploadCommandOutput } from "@aws-sdk/client-s3";
...
export const getSignedURLFromS3 = async ({ s3Bucket, fileName, expiresIn = ms("24h") / 1000 }: GetFileOptions): Promise<string | null> => {
  try {
...
export const getCloudFrontURLFromS3 = async ({ fileName, isSigned, dateLessThan = moment().add(1, "days") }: GetCloudFrontURLOptions) => {
  try {
